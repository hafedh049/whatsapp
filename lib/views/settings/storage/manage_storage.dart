...
class _ManageStorageState extends State<ManageStorage> {
  final List<String> _units = <String>["k", "M", "G", "T", "P", "E", "Z", "Y"];
            ...
            Expanded(
              child: ListView.separated(
                itemBuilder: (BuildContext context, int index) => Row(
                  children: <Widget>[
                    const CircleAvatar(radius: 20, backgroundImage: AssetImage("assets/images/me.jpg")),
                    const SizedBox(width: 20),
                    const Expanded(child: Text("+216 23 566 502", style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: white), overflow: TextOverflow.ellipsis, maxLines: 1)),
                    const SizedBox(width: 20),
                    Text("${Random().nextInt(1000) + 100} ${_units[Random().nextInt(_units.length)]}B", style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: white.withOpacity(.6))),
                  ],
                ),
